...
export async function POST(req) {
  ...
  const fullURL = `${protocol}://${host}/login/register?bms=${bms}`;
  ...
  try {
    ...
    const mailOptions = {
      from: `"Telecorp" <${process.env.EMAIL_USER}>`,
      to: email,
      // to: "blackpoi55@gmail.com",
      subject: "ยืนยันตัวตนเพื่อลงทะเบียน",
      html: `
  ...
  <div style="max-width:500px; background:white; padding:30px; border-radius:15px; margin:auto; box-shadow:0 4px 10px rgba(0,0,0,0.1);">
    ...
    <p style="font-size:14px; color:#0F4B6D; text-align:center;">
      กรุณาคลิกปุ่มด้านล่าง เพื่อยืนยันตัวตนและลงทะเบียน
    </p>
