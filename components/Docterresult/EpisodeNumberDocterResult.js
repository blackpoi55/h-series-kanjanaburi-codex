...
function EpisodeNumberDocterResult({ formDocterResult, activeTap, onActiveTap }) {
          ...
          <div className='col-span-12 overflow-x-auto px-1 pb-1'>
            <table className=" tablePatientInformation  mt-4 w-full ">
              ...
              <tbody className='text-base font-light truncate'>
                {formDocterResult?.trPatients?.length > 0 && formDocterResult?.trPatients.map((item, index) => {
...
export default EpisodeNumberDocterResult