...
function Header(props) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      // สร้าง array สำหรับข้อมูลทุกๆ sheet
      const allSheetsData = workbook.SheetNames.map(sheetName => {
        ...
        const fixeddataMap = rawdataMap.slice(1).map(row => {
          ...
          const rowdataMap = headers.reduce((acc, header, index) => {
            if ((header === "exam_date" || header === "birth_date" || header === "nurse_exam_date") && row[index]) {
              ...
              if (typeof dateSerial === "number") {
                ...
                const year = parseInt(excelDate.getUTCFullYear()) > 2500 ? parseInt(excelDate.getUTCFullYear()) - 543 : excelDate.getUTCFullYear(); // ปรับปีพุทธศักราชเป็นคริสต์ศักราช 
                // const year = excelDate.getUTCFullYear();// ปรับปีพุทธศักราชเป็นคริสต์ศักราช
                acc[header] = `${year}-${month}-${day}`;
