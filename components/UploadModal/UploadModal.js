...
export default function UploadModal({ open, onClose, healthrecordsidRef, onSaved }) {
    ...
    const saveClick = async () => {
        ...
        let data = await posttrhealthfiles(healthrecordsidRef.current, val);

        if (!data?.error) {
            if (typeof onSaved === 'function') {
                onSaved(); // ✅ เรียก refresh() ที่ส่งมาจาก parent
            }
            Swal.fire({
                icon: 'success',
                title: 'บันทึกสำเร็จ',
            ...
            // รอให้ Swal แสดงซักแป๊บก่อนปิด Dialog และดึงข้อมูลใหม่
            setTimeout(() => {
                onClose();
            }, 1600);
        }
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
            <DialogTitle onClick={() => console.log(fileList)}>📤 อัปโหลดไฟล์ PDF</DialogTitle>
            <DialogContent>
                <Box
                    onClick={() => inputRef.current.click()}
                    className="mb-4 w-full max-w-xl mx-auto border-2 border-dashed border-gray-400 rounded-xl p-6 text-center cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition"
                >
                ...
                <Box className="mt-4 flex flex-col gap-2">
                    {fileList && fileList.length > 0 && fileList.map((file, index) => (
                        <Box key={index} className="flex items-center justify-between bg-gray-100 px-3 py-2 rounded shadow-sm">
                            <Typography className="truncate max-w-[60%]">{file.original_filename}</Typography>
                            <Box className="flex items-center gap-1">
                                <IconButton color="error" onClick={() => removeFile(index)}>
                                    <DeleteIcon />
                                </IconButton>
                                {index > 0 && (
                                    <IconButton onClick={() => moveFile(index, index - 1)}>
                                        <ArrowUpwardIcon />
                                    </IconButton>
                                )}
            ...
            <div className="flex justify-end items-center gap-3 p-4 bg-gray-100 rounded-b-lg">
                <button
                    onClick={onClose}
