...
import { addPatienthistory, addVitalsign, getPatientHistoryById, getVitalsignById, searchCategory, updatePatienthistory, updatePhysicalexam, updateVitalsign } from '@/action/api'
import { MetaPhysicalBlood, PhysicalBlood } from '../Tool/var'

function VitalSign({ UID, activeTap, onActiveTap }) {
  ...
  const refresh = async () => {
    try {
      ...
      if (resPatientHistory?.message === 'success') {
        let data = resPatientHistory?.data || []
        setFormPatientHistory({
          ...data,
          aa7: data?.PassDisease
        })
      } else {
  ...
  const handleChangeHistory = (update) => {
    setFormPatientHistory({ ...formPatientHistory, ...update })
  }

  const handleChangePatientHistory = (update) => {
    if (update.hasOwnProperty('PresentIllness')) {
      update.PersonalHistoryPresentIllness = update?.PresentIllness
    } else if (update.hasOwnProperty('Drugs')) {
      update.PersonalHistoryCurrentMedication = update?.Drugs
    ...
    } else if (update.hasOwnProperty('PersonalHistoryAdmission')) {
      update.PersonalHistoryAdmission_detail = update?.PersonalHistoryAdmission
    } else if (update.hasOwnProperty('PassDisease')) {
      update.PresentDisease = update?.PassDisease
    }

    if (update.hasOwnProperty('_PHComment')) {
      if (update?._PHComment) {
        update.PHComment = (formPatientHistory?.PHComment || '') + (formPatientHistory?.PHComment ? '\n' : '') + update?._PHComment
      }
    }

    //3.1 ประวัติการสูบบุหรี่
    if (update.SmokingNone) {
      formPatientHistory.SmokingVolumes = '',
        formPatientHistory.SmokingYears = '',
        formPatientHistory.SmokingQuitDetail2 = '',
        formPatientHistory.SmokingQuitOther = '',
        formPatientHistory.SmokingYear = '',
        formPatientHistory.SmokingMonth = '',
        formPatientHistory.SmokingRemark = ''
    }
    //3.2 ประวัติการดื่มแอลกอฮอล์
    if (update.Alcohol) {
      formPatientHistory.AlcoholYears = '',
        formPatientHistory.AlcoholVolume = '',
        formPatientHistory.AlcoholicDrinkYear = '',
        formPatientHistory.AlcoholicDrinkMonth = ''
    }
    //3.3 ประวัติการออกกำลังกาย
    if(update.Exercise){
      formPatientHistory.ExerciseType = ''
    }
    setFormPatientHistory({ ...formPatientHistory, ...update })
  }
    ...
    clearAlert({
      ...
      onSave: () => {
        ...
        setForm((p) => ({
          ...p, ...{
            Height: null,
            ChestInspiration: null,
            ChestExpiration: null,
            Weight: null,
            BMI: null,
            BMIDetail: null,
            BPSys: null,
    ...
    saveAlert({
      ...
      onSave: async () => {
        ...
        const saveData = {
          ...
          ChestInspiration: form?.ChestInspiration || null,
          ChestExpiration: form?.ChestExpiration || null,
    ...
    saveAlert({
      ...
      onSave: async () => {
        ...
        const saveData = {
          ...
          Drugs: formPatientHistory?.Drugs || null,
          PHComment: formPatientHistory?.PHComment || null,
          ...
          PersonalHistoryAdmission_detail: formPatientHistory?.PersonalHistoryAdmission_detail || null,
          ...
          //เพิ่มมาใหม่
          // MHHeart: formPatientHistory?.Heart || null,
          // PersonalHistoryPassDisease: formPatientHistory?.txtPassDisease || null,
          // SmokingQuitDetail: formPatientHistory?.TextBoxSmokingQuitDetail || null,
          // SmokingOther: formPatientHistory?.txtSmokingDetail || null,
          // SmokingNoneLong: formPatientHistory?.rdLongtimeSmoking || null,
          // SmokingNoneLongYear: formPatientHistory?.txtLongtimeSmokingYear || null,
          // SmokingNoneLongDetail: formPatientHistory?.txtLongtimeSmokingDetail || null,
          // AlcoholDetail: formPatientHistory?.txtAlcoholDetail || null,
          // AlcoholNoneLong: formPatientHistory?.rdLongtimeAlcohol || null,
          // AlcoholNoneLongYear: formPatientHistory?.txtLongtimeAlcoholYear || null,
          // AlcoholNoneLongDetail: formPatientHistory?.txtLongtimeAlcoholDetail || null,
          // Immunity: formPatientHistory?.Immunity || null,
          // ImmunityDetail: formPatientHistory?.ImmunityDetail || null,
          // Drugs: formPatientHistory?.Drugs || null,
          // HistoryOther: formPatientHistory?.TextBoxPH_Other || null,
          // PHComment: formPatientHistory?.TextBoxCD_Detail || null,
          // MHAsthma: formPatientHistory?.Asthma || null,
          // MHHypertension: formPatientHistory?.Hypertension || null,
          // MHHemoptysis: formPatientHistory?.Hemoptysis || null,
          // MHDiabetes: formPatientHistory?.Diabetes || null,
          // MHJaundice: formPatientHistory?.Jaundice || null,
          // MHEpilepsy: formPatientHistory?.Epilepsy || null,
          // MHVeneral: formPatientHistory?.Veneral || null,
          // MHAcquired: formPatientHistory?.Acquired || null,
          // MHEdema: formPatientHistory?.Edema || null,
          // MHYaws: formPatientHistory?.Yaws || null,
          // MHOtorrhea: formPatientHistory?.Otorrhea || null,
          // MHHernia: formPatientHistory?.Hernia || null,
          // MHHEmorrhoid: formPatientHistory?.HEmorrhoid || null,
          // MHAccident: formPatientHistory?.Accident || null,
          // MHFractures: formPatientHistory?.Fractures || null,
          // MHSurgical: formPatientHistory?.Surgical || null,
          // MHMalaria: formPatientHistory?.Malaria || null,
          // MHLMP: formPatientHistory?.TextBoxLMP || null,
          // MHDetail: formPatientHistory?.TextBoxMHDetail || null,
          // job_description: formPatientHistory?.TextBoxjob_description || null,
          // SmokingRemark: formPatientHistory?.TextBoxSmokingOther || null,
        }
        try {
          startLoading()
  ...
  return (
    <div className='w-full flex flex-col rounded-2xl bg-[#F8F8F8] shadow-box p-5 my-4'>
      ...
      <Collapse timeout={300} className="transition-all duration-500" in={switchStaus}>
        <div className='grid grid-cols-12  items-start bg-[#F3F3F3] rounded-lg shadow-box w-full gap-2 mt-4 p-2'>
          ...
          <div className='grid grid-cols-12  col-span-12 gap-4 mt-2'>
            ...
            <div className='flex gap-2 col-span-12'>
              <div className='flex gap-2 whitespace-nowrap'>
                <input type="checkbox" className='w-5 h-5 accent-[#365382]' id='other-nationality1' name="other-nationality" value="" />
                <label htmlFor='other-nationality1' className='font-medium'>สัญชาติอื่นๆ</label>
              </div>
              <TextareaAutosize minRows={3} maxRows={5} value={form?.BMIDetail || ''} onChange={(BMIDetail) => handleChange({ BMIDetail: BMIDetail?.target?.value })} className="w-full rounded-lg h-[80px] p-2" aria-label="Demo input" placeholder=""
              />
            </div>
          </div>
          {/* เส้นขั้น */}
          ...
          <div className='grid grid-cols-12  col-span-12 gap-4'>
            ...
            <div className='relative flex col-span-4 ml-1 bg-[#FFFFFF]'>
              <Toolselect2 sm options={options?.Blood || []} label={"เลือด"} value={form?.BPDetail || ''} onChange={(BPDetail) => handleChange({ BPDetail })} />
            </div>


            <TextareaAutosize minRows={3} maxRows={5} value={form?.BPDetail || ''} onChange={(BPDetail) => handleChange({ BPDetail: BPDetail?.target?.value })} className="col-span-12 rounded-lg h-[80px] p-2" aria-label="Demo input" placeholder="" />
            ...
            <div className='relative flex col-span-8 ml-1 bg-[#FFFFFF]'>
              <TextField size='small' className='text-white w-full' value={form?.PulseDetail || ''} onChange={(PulseDetail) => handleChange({ PulseDetail: PulseDetail?.target?.value })} label="Pulse Detail" variant="outlined" />
            </div>
            ...
            <div className='relative flex col-span-4 ml-1 bg-[#FFFFFF]'>
              <TextField size='small' className='text-white w-full' value={form?.ChestExpiration || ''} onChange={(ChestExpiration) => handleChange({ ChestExpiration: ChestExpiration?.target?.value })} label="หายใจออก" variant="outlined" />
            </div>
          ...
          <div className='grid grid-cols-12 col-span-12 items-end gap-2'>
            <div className='relative flex lg:flex-row flex-col col-span-4 ml-1 gap-2 items-end'>
              <div className='bg-[#FFFFFF] flex  w-full'>
                <Toolselect2 sm options={MetaPhysicalBlood || []} label={"สรุปผลการตรวจร่างกายโดยแพทย์"} value={form?.Remarks || ''} onChange={(Remarks) => handleChange({ Remarks })} />
              </div>
              {/* <button className='h-10 lg:w-[30%] w-full border rounded-lg bg-[#6B84B7] hover:bg-[#586f9e] text-[#FFFFFF]' >เพิ่ม</button> */}
            </div>
            <div className='relative flex col-span-8 ml-1 gap-2 items-end'>
              <div className='bg-[#FFFFFF] flex w-full  rounded-lg'>
                <TextareaAutosize minRows={3} maxRows={5} value={form?.Remarks || ''} onChange={(e) => handleChange({ Remarks: e?.target?.value })} className=" w-full rounded-lg p-2" aria-label="Demo input" placeholder="" />
              </div>
        ...
        {/* Patient History */}
        <div className='grid grid-cols-12  items-start bg-[#F3F3F3] rounded-lg shadow-box w-full gap-2 mt-4 p-2'>
          <label className='font-semibold text-[#365382] text-base col-span-12' >Patient History</label>
          ...
          <div className='grid grid-cols-12 col-span-12 items-end gap-2 text-base font-light'>
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
                ...
                <div className='flex justify-start items-center gap-4 w-full col-span-12 lg:col-start-3'>
                  ...
                  <div className='relative grid grid-cols-12 w-full gap-2 items-end'>
                    <div className='bg-[#FFFFFF] flex col-span-12'>
                      <Toolselect2 sm options={options?.Disease || []} value={formPatientHistory?._PHComment} onChange={(_PHComment) => handleChangePatientHistory({ _PHComment })} />
                    </div>
              ...
              <div className='ml-6  lg:ml-0 col-span-12 lg:col-span-8 flex gap-2 items-end'>
                <TextareaAutosize value={formPatientHistory?.PHComment || ''} onChange={(e) => handleChangePatientHistory({ PHComment: e?.target.value })} minRows={2} maxRows={2} className="w-full rounded-lg h-f p-2" aria-label="" placeholder="" />
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
              <div className='lg:col-span-4 flex  col-span-12 lg:grid lg:grid-cols-12 gap-2'>
                ...
                <div className='flex justify-start items-center gap-4 col-span-12 col-start-6'>
                  <div className='flex gap-2 items-center'>
                    <input checked={formPatientHistory.Drugs === 'ไม่มี'} onChange={() => handleChangePatientHistory({ Drugs: "ไม่มี" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="Drugsnot" name="Drugs" value="" />
                    <label htmlFor='Drugsnot' className=''>ไม่มี</label>
                  </div>
                  <div className='flex gap-2 items-center'>
                    <input checked={formPatientHistory.Drugs === 'มี'} onChange={() => handleChangePatientHistory({ Drugs: "มี" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="Drugshave" name="Drugs" value="" />
                    <label htmlFor='Drugshave' className=''>มี</label>
                  </div>
                </div>

              </div>
              <div className='ml-6  lg:ml-0 col-span-12 lg:col-span-8 flex gap-2 items-end'>
                <TextareaAutosize disabled={formPatientHistory.Drugs === 'ไม่มี'} value={formPatientHistory?.PersonalHistoryCurrentMedication || ''} onChange={(PersonalHistoryCurrentMedication) => handleChangePatientHistory({ PersonalHistoryCurrentMedication: PersonalHistoryCurrentMedication?.target.value })} minRows={2} maxRows={2} className="w-full rounded-lg h-f p-2" aria-label="" placeholder="" />
              </div>
            </div>
          ...
          <div className='grid grid-cols-12 col-span-12 items-end gap-2 text-base font-light'>
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
              <div className='lg:col-span-4 flex  col-span-12 lg:grid lg:grid-cols-12 gap-2'>
                ...
                <div className='flex justify-start items-center gap-4 col-span-12 col-start-6'>
                  <div className='flex gap-2 items-center'>
                    <input checked={formPatientHistory.PersonalHistoryAdmission === 'ไม่มี'} onChange={() => handleChangePatientHistory({ PersonalHistoryAdmission: "ไม่มี" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="Admissionnot" name="Admission" value="" />
                    <label htmlFor='Admissionnot' className=''>ไม่มี</label>
                  </div>
                  <div className='flex gap-2 items-center'>
                    <input checked={formPatientHistory.PersonalHistoryAdmission === 'มี'} onChange={() => handleChangePatientHistory({ PersonalHistoryAdmission: "มี" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="Admissionhave" name="Admission" value="" />
                    <label htmlFor='Admissionhave' className=''>มี</label>
                  </div>
                </div>
              </div>
              <div className='ml-6  lg:ml-0 col-span-12 lg:col-span-8 flex gap-2 items-end'>
                <TextareaAutosize disabled={formPatientHistory.PersonalHistoryAdmission === 'ไม่มี'} value={formPatientHistory?.PersonalHistoryAdmission_detail || ''} onChange={(PersonalHistoryAdmission_detail) => handleChangePatientHistory({ PersonalHistoryAdmission_detail: PersonalHistoryAdmission_detail?.target.value })} minRows={2} maxRows={2} className="w-full rounded-lg  p-2" aria-label="" placeholder="" />
              </div>
            </div>
          ...
          <div className='grid grid-cols-12 col-span-12 items-end gap-2 text-base font-light'>
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
              ...
              <div className='flex lg:flex-row flex-col col-start-3 items-start lg:items-center gap-4  col-span-12'>
                ...
                <div className='flex gap-2 items-center'>
                  ...
                  <div className='relative flex col-span-4 items-center ga  gap-4 whitespace-nowrap'>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เคยและยังสูบอยู่ปริมาณ"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingVolumes || ''} onChange={(SmokingVolumes) => handleChangePatientHistory({ SmokingVolumes: SmokingVolumes?.target.value })} label="" variant="outlined" />
                    <label>มวน/วัน</label>
                  </div>
                </div>
              ...
              <div className='flex lg:flex-row flex-col col-start-3 items-start lg:items-center gap-4  col-span-12'>
                <div className='flex gap-2 items-center'>
                  <input checked={formPatientHistory.SmokingNone === "เคยแต่เลิกสูบแล้ว ระยะที่เคยสูบนาน"} onChange={() => handleChangePatientHistory({ SmokingNone: "เคยแต่เลิกสูบแล้ว ระยะที่เคยสูบนาน" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="SmokingLong1" name="SmokingLong" value="" />
                  ...
                  <div className='relative flex col-span-4 items-center'>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เคยแต่เลิกสูบแล้ว ระยะที่เคยสูบนาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingYears || ''} onChange={(SmokingYears) => handleChangePatientHistory({ SmokingYears: SmokingYears?.target.value })} label="" variant="outlined" />
                    <label className='ml-2'>ปี</label>
                  </div>
                  <div className='relative flex col-span-4 items-center'>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เคยแต่เลิกสูบแล้ว ระยะที่เคยสูบนาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingQuitDetail2 || ''} onChange={(SmokingQuitDetail2) => handleChangePatientHistory({ SmokingQuitDetail2: SmokingQuitDetail2?.target.value })} label="" variant="outlined" />
                ...
                <div className='relative flex col-span-4  items-center gap-2 whitespace-nowrap'>
                  ...
                  <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เคยแต่เลิกสูบแล้ว ระยะที่เคยสูบนาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingQuitOther || ''} onChange={(SmokingQuitOther) => handleChangePatientHistory({ SmokingQuitOther: SmokingQuitOther?.target.value })} label="" variant="outlined" />
                  <label>มวน/วัน</label>
                </div>
              </div>
              ...
              <div className='flex col-start-3 items-center gap-4  col-span-12'>
                <div className='flex gap-2 items-center'>
                  <input checked={formPatientHistory.SmokingNone === "เลิกสูบมานาน"} onChange={() => handleChangePatientHistory({ SmokingNone: "เลิกสูบมานาน" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="smoke3_1" name="smoke3" value="" />
                  <label htmlFor='smoke3_1' className=''>เลิกสูบมานาน</label>
                  <div className='relative flex col-span-4  items-center'>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เลิกสูบมานาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingYear || ''} onChange={(SmokingYear) => handleChangePatientHistory({ SmokingYear: SmokingYear?.target.value })} label="" variant="outlined" />
                    <label className='ml-2'>ปี</label>
                  </div>
                  <div className='relative flex col-span-4 items-center '>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "เลิกสูบมานาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.SmokingMonth || ''} onChange={(SmokingMonth) => handleChangePatientHistory({ SmokingMonth: SmokingMonth?.target.value })} label="" variant="outlined" />
                    <label className='ml-2'>เดือน</label>
                  </div>
                </div>
              </div>
              ...
              <div className='flex col-start-3 items-center gap-4  col-span-12'>
                <div className='flex gap-2 items-center'>
                  <input checked={formPatientHistory.SmokingNone === "อื่นๆ"} onChange={() => handleChangePatientHistory({ SmokingNone: "อื่นๆ" })} className='w-4 h-4 min-w-4 min-h-4 accent-[#365382]' type="radio" id="smoke-other1" name="smoke-other" value="" />
                  <label htmlFor='smoke-other1' className=''>อื่นๆ</label>
                  <div className='relative flex col-span-4  items-center'>
                    <TextField size='small' disabled={formPatientHistory.SmokingNone !== "อื่นๆ"} className='text-white w-full bg-[#FFFFFF]' value={formPatientHistory?.SmokingRemark || ''} onChange={(SmokingRemark) => handleChangePatientHistory({ SmokingRemark: SmokingRemark?.target.value })} label="" variant="outlined" />
                  </div>
                </div>
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
              ...
              <div className='flex lg:flex-row flex-col col-start-3 items-start lg:items-center gap-4 col-span-12'>
                ...
                <div className='flex gap-2 items-center'>
                  ...
                  <div className='relative flex col-span-4 items-center gap-2 whitespace-nowrap'>
                    <TextField size='small' disabled={formPatientHistory.Alcohol !== "เลิกดื่ม"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.AlcoholYears || ''} onChange={(AlcoholYears) => handleChangePatientHistory({ AlcoholYears: AlcoholYears?.target.value })} label="" variant="outlined" />
                    <label className=''>ปี</label>
                  </div>
                  <div className='relative flex col-span-4 items-center gap-2 whitespace-nowrap'>
                    <TextField size='small' disabled={formPatientHistory.Alcohol !== "เลิกดื่ม"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.AlcoholVolume || ''} onChange={(AlcoholVolume) => handleChangePatientHistory({ AlcoholVolume: AlcoholVolume?.target.value })} label="" variant="outlined" />
              ...
              <div className='flex lg:flex-row flex-col col-start-3 items-start lg:items-center gap-4 col-span-12'>
                <div className='flex gap-2 items-center'>
                  ...
                  <div className='relative flex col-span-4 gap-2  items-center'>
                    <TextField size='small' disabled={formPatientHistory.Alcohol !== "เคยแต่เลิกดื่มแล้ว ระยะที่เคยดื่มนาน"} className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.AlcoholicDrinkYear || ''} onChange={(AlcoholicDrinkYear) => handleChangePatientHistory({ AlcoholicDrinkYear: AlcoholicDrinkYear?.target.value })} label="" variant="outlined" />
                    <label>ปี</label>
                  </div>
                  <div className='relative flex col-span-4 gap-2 items-center'>
                    <TextField size='small' disabled={formPatientHistory.Alcohol !== "เคยแต่เลิกดื่มแล้ว ระยะที่เคยดื่มนาน"}className='text-white w-20 bg-[#FFFFFF]' value={formPatientHistory?.AlcoholicDrinkMonth || ''} onChange={(AlcoholicDrinkMonth) => handleChangePatientHistory({ AlcoholicDrinkMonth: AlcoholicDrinkMonth?.target.value })} label="" variant="outlined" />
                    <label>เดือน</label>
                  </div>
                </div>
            ...
            <div className='col-span-12 gap-2 grid grid-cols-12 '>
              ...
              <div className='flex lg:flex-row flex-col col-start-3 items-start lg:items-center gap-4 col-span-12'>
                ...
                <div className='flex gap-2 items-center'>
                  ...
                  <div className='relative flex col-span-4 items-center ga  gap-4 whitespace-nowrap'>
                    <label>ระบุ</label>
                    <TextField size='small' label={'ระบุชนิดกีฬา'} disabled={formPatientHistory.Exercise !== "สม่ำเสมอ"}className='text-white col-span-4 bg-[#FFFFFF]' value={formPatientHistory?.ExerciseType || ''} onChange={(ExerciseType) => handleChangePatientHistory({ ExerciseType: ExerciseType?.target.value })} variant="outlined" />
                  </div>
                </div>
              </div>
            </div>
