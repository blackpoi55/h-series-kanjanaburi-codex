...
function Tablewaiting() {
    ...
    const refresh = async () => {
        ...
        if (res?.message === 'success') {
            const filteredData = res?.data?.filter(x => x.nurse_approve === null);
                    ...
                    <tbody className='text-base font-light'>
                        {list.length > 0 && list.map((item, index) => (
                            <tr key={`trPatientLab${index}`} className="hover">
                                ...
                                <td className={`text-left ${item?.nurse_approve === null ? 'text-orange-500' : ''}`}>
                                    {item?.nurse_approve === null ? 'รอยืนยันผล' : ''}
                                </td>
                            </tr>
                        ))}
                    </tbody>
