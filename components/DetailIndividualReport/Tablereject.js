...
function Tablereject(props) {
    ...
    const handleOpenNote = (note, name) => {
        setnameNote(name)
        setNoteText(note);
        setNoteOpen(true);
    };
    ...
    const refresh = async () => {
        ...
        try {
            ...
            if (res?.message === 'success') {
                ...
                if (userData === "nurse") {
                    filteredData = res?.data?.filter(x => x.reject_status == "Y") || [];
    ...
    return (
        <div className='flex flex-col col-span-12 lg:col-span-10 justify-start gap-2 p-4 rounded-lg shadow-box bg-[#FFFFFF]'>
            <NoteModal name={"หมายเหตุที่ปฏิเสธจากหมอ"} open={noteOpen} onClose={handleCloseNote} noteText={noteText} />
                ...
                <table className="tablePatientInformation w-full">
                    <thead className='text-[#4E4E4E]  text-sm z-10'>
                        <tr className="text-left bg-[#E2E2E2]">
                            ...
                            <th className="font-light text-center">Doctor Note</th>
                            ...
                            displayedData.map((item, index) => (
                                <tr key={`trPatientLab${index}`} className="hover">
                                    ...
                                    <td className=" text-center">
                                        <button disabled={!item?.nurse_note} onClick={() => handleOpenNote(item.nurse_note, "หมายเหตุจากพยาบาล")} className="w-8 h-8 border rounded-full text-white bg-[#6B84B7] hover:bg-[#5c75a0] disabled:bg-gray-300">
                                            📝
                                        </button>
                                    </td>
                                    <td className=" text-center">
                                        <button disabled={!item?.doctor_note} onClick={() => handleOpenNote(item.doctor_note, "หมายเหตุที่ปฏิเสธจากหมอ")} className="w-8 h-8 border rounded-full text-white bg-[#6B84B7] hover:bg-[#5c75a0] disabled:bg-gray-300">
                                            📝
                                        </button>
