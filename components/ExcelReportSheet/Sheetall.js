export const Sheetall = async (companyName, name, data, workbook) => {
    console.log(companyName,name,data)
    ...
    const rows = [
        { col1: "ลำดับ", col2: "NO", col3: "HN", col4: "ชื่อ - นามสกุล", col5: "รหัสพนักงาน", col6: "แผนก", col7: ((data?.[0]?.trLabs?.[0]?.ItemDesc || "") + " ( " + (data?.[0]?.trLabs?.[0]?.LabRange || "-") + " ) "), col8: "ผลการตรวจ" },
        // { col7: "70-99 mg/dL" },
        // {},
    ];
    ...
    for (let index = 0; index < data.length; index++) {
        ...
        worksheet.addRow({
            ...
            col3: getdata.HN || '-',
            col4: (getdata.Prename || "") + " " + (getdata.Forename || "") + " " + (getdata.Surname || ""),
            col5: getdata.EmpID || '-',
            col6: getdata.Dept || '-',
            col7: getdata?.trLabs?.[0]?.TestData || '-',
            col8: getdata?.trLabs?.[0]?.TranslateResult || '-'
        });
    }
