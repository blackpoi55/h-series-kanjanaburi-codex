...
function ModalComponent({ headname, classheadname = null, isModal, children, z = 100, modalsize, setIsModal, onSave, onCancel, button = null, footCenter, x = true }) {
                ...
                {checkFooter && <div style={{ zIndex: z + 1 }} className={`absolute bottom-0  rounded-b-lg left-0 bg-white h-16 w-full md:px-4 px-2 flex ${footCenter ? 'justify-center' : 'justify-end'} items-center gap-2 border-t`}>
                    {button ? button
                        : <>
                            ...
                            {onSave && <button onClick={() => onSave()} className="z-auto h-10 w-40 py-1 shadow-md text-base text-white rounded-lg bg-[#365382] hover:bg-[#0B2756]">
                                <div className='flex gap-2 justify-center items-center' >
                                    <img style={{ zIndex: z + 1 }} className=' cursor-pointer' width={24} height={24} src="/icon/save.svg" />
